...
Properties {
	...
	float4 frag(v2f i) : SV_Target {
		...
		for(int j = 84 ; j > 0; j--){
			...
			if(abs(xp - lol) < 0.1 && xp < (1.0/_Cam0_TexelSize.x)) {
			...
			if(abs(xp1 - lol) < 0.1 && xp1 < (1.0/_Cam0_TexelSize.x)) {
		...
		if(tmpCam1Value.w > realCamera0Colors.w){ 
			//return tmpCam0Value;
		}
		return tmpCam0Value;
